...
function getPicoIp() {
  return localStorage.getItem("picoIp") || "";
}

function savePicoIp() {
  const ip = document.getElementById("pico_ip").value;
  if (ip) {
    localStorage.setItem("picoIp", ip);
    alert(`Pico IP saved: ${ip}`);
  } else {
    alert("Please enter a valid IP address");
  }
}

async function sendPicoCommand(endpoint, method = "POST") {
  const ip = getPicoIp();
  if (!ip) {
    alert("Please set the Pico IP address first");
    return null;
  }
  ...
  try {
    ...
    console.error("Pico command error:", err);
    alert(`Failed to communicate with Pico: ${err.message}`);
    return null;
  }
}

async function getStatus() {
  const data = await sendPicoCommand("/status", "GET");
  if (data) {
    const statusDiv = document.getElementById("pico_status");
    const contentDiv = document.getElementById("status_content");
    contentDiv.innerHTML = `
      <p>Foil Threshold: ${data.foil_thresh}</p>
      <p>Foil DRY Value: ${data.foil_dry}</p>
      <p>Foil WET Value: ${data.foil_wet}</p>
      <p>Temperature Limit: ${data.temp_limit}°C</p>
    `;
...
async function resetThreshold() {
  const data = await sendPicoCommand("/reset_threshold");
  if (data && data.success) {
    alert(`Threshold reset to default: ${data.new_threshold}`);
    getStatus();
  }
}

async function setThreshold() {
  const value = document.getElementById("threshold_value").value;
  if (!value) {
    alert("Please enter a threshold value");
    return;
  }

  const data = await sendPicoCommand(`/set_threshold?value=${value}`);
  if (data && data.success) {
    alert(`Threshold set to: ${data.new_threshold}`);
    getStatus();
  }
}
...
async function calibrateDry() {
  if (
    !confirm(
      "Make sure the sensor is DRY right now. Continue with calibration?"
    )
  ) {
    return;
  }

  const data = await sendPicoCommand("/calibrate_dry");
  if (data && data.success) {
    alert(
      `DRY calibration complete!\nDRY value: ${data.foil_dry}\nNew threshold: ${data.new_threshold}`
    );
    getStatus();
  }
}
...
async function calibrateWet() {
  if (
    !confirm(
      "Make sure the sensor is WET right now. Continue with calibration?"
    )
  ) {
    return;
  }

  const data = await sendPicoCommand("/calibrate_wet");
  if (data && data.success) {
    alert(
      `WET calibration complete!\nWET value: ${data.foil_wet}\nNew threshold: ${data.new_threshold}`
    );
    getStatus();
  }
}

// Load saved Pico IP on page load
window.addEventListener("DOMContentLoaded", () => {
  const savedIp = getPicoIp();
  if (savedIp) {
    document.getElementById("pico_ip").value = savedIp;
  }
});
